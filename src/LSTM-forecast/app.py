...
def forecast():
    ...
    df1 = data_frame.groupby([types], as_index=False).count()
    df = df1[[types, 'issue_number']]
    df.columns = ['ds', 'y']
    ...
    size = 0.80
    if type == "commit_issues" :
        size = 0.50
    train_size = int(len(Ys) * size)
...
@app.route('/api/stats/forecast', methods=['POST'])
def stat_forecast() :
    body = request.get_json()
    issues = body["issues"]
    type = body["type"]
    ...
    # Creating the image path for model loss, LSTM generated image and all issues data image
    STATS_FORECAST = "stats_forecast_" + type + ".png"
    STATS_FORECAST_URL = BASE_IMAGE_PATH + STATS_FORECAST

    STATS_FORECAST_FIT = "stats_forecast_fit" + type + ".png"
    STATS_FORECAST_FIT_URL = BASE_IMAGE_PATH + STATS_FORECAST_FIT

    data_frame = pd.DataFrame(issues)
    data_frame.set_index('count')
    # predict = sm.tsa.seasonal_decompose(data_frame.index.values,freq=10)
    # fig, axs = plt.subplots(1, 1, figsize=(10, 4))
    # axs = predict.plot()
    # plt.savefig(LOCAL_IMAGE_PATH + STATS_FORECAST)
    # For build 

    model = sm.tsa.ARIMA(data_frame['count'].iloc[1:] , order=(1,0,0))
    res = model.fit()
    data_frame['forecast'] = res.fittedvalues
    fig, test = plt.subplots(1, 1, figsize=(10, 4))
    test = data_frame[['count','forecast']].plot()
    plt.savefig(LOCAL_IMAGE_PATH + STATS_FORECAST_FIT)
 
    

    bucket = client.get_bucket(BUCKET_NAME)
    # new_blob = bucket.blob(STATS_FORECAST)
    # new_blob.upload_from_filename(
    #     filename=LOCAL_IMAGE_PATH + STATS_FORECAST)
    new_blob = bucket.blob(STATS_FORECAST_FIT)
    new_blob.upload_from_filename(
        filename=LOCAL_IMAGE_PATH + STATS_FORECAST_FIT)

    json_response = {
        # "stat_forecast_image_url": STATS_FORECAST,
        "stat_forecast_fit_image_url": STATS_FORECAST_FIT_URL
    }
    # Returns image url back to flask microservice
    return jsonify(json_response)
